...
def process_data(data: Dict[str, pl.DataFrame], _: Any) -> Dict[str, pl.DataFrame]:
    ...
    processed_df = df.filter(
        pl.col("data").bin.starts_with(bytes([228, 69, 165, 46, 81, 203, 154, 29, 64, 198, 205, 232, 38, 8, 113, 226]))
        & pl.col("program_id").bin.starts_with(bytes([4, 121, 213, 91, 242, 49, 192, 110, 238, 116, 197, 110, 206, 104, 21, 7, 253, 177, 178, 222, 163, 244, 142, 81, 2, 177, 205, 162, 86, 188, 19, 143]))
    )
            ...
            cc.Step(
                ...
                config=cc.SvmDecodeInstructionsConfig(
                    instruction_signature=InstructionSignature(
                        discriminator = base58.b58encode(bytes([228, 69, 165, 46, 81, 203, 154, 29, 64, 198, 205, 232, 38, 8, 113, 226])).decode(),
                        params = [
                            ParamInput(
                                name = "Amm",
